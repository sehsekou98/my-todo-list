...
function Board() {
  const [board, getBoard, setBoardState, updateTodoInDB] = useBearStore((state) => [
    state.board,
    state.getBoard,
    state.setBoardState,
  ...
  const handleDnDragEnd = (result: DropResult) => {
    ...
    // Check if user draggrd card outside of board
    ...
    // This step is needed as the indexes are store as numbers. instead of id's with DND library.
